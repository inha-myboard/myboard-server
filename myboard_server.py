...
class inspectorAPI(Resource):
  def get(self, apiId):
    # _apiId = apiId
    query = "SELECT api_json FROM myboard.api WHERE id=%s" % apiId
    getjson = selectSQL(query)[0]
    print(getjson['api_json'])
    return(inspector(str(getjson['api_json'])))

class widgetAPI(Resource):
  def get(self, widgetID): #list, search  #뒤에 변수값 있으면 search, null이면 list
    query = "SELECT id,api_id,user_id,caption,description,mapping_json from myboard.widget where id=%s" % widgetID
    return(selectSQL(query))
...
class dashboardAPI(Resource):
    def get(self,user, id): #dashboard list
        jsondata = request.get_json(force=True)
        _apiUser_id = jsondata['user_id']
        # _apiId = jsondata['id']
...
class dashboardAPIList(Resource):
    def get(self, user): #dashboard list
        query = "SELECT id, name, order_index FROM myboard.dashboard WHERE user_id=%s"
        return(executeSQL(query, (user)))
...
api.add_resource(dashboardAPI, '/dashboards/<id>') # //dash id 입력하면 그 대쉬보드를 // get,post,del, put
api.add_resource(dashboardAPIList, '/users/<user>/dashboards') # user의 dash list가져오기 //get
# api.add_resource(dashboardAPIList, '/dashboards/<id>/widgets/') # 대시보드 위젯리스트 조회, 저장 //get, post
